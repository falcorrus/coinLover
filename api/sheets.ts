...
export default async function handler(req, res) {
  ...
  // Resolve ssId if it is a contact handle (e.g. starts with @ or does not match spreadsheet ID format)
  if (ssId && ssId !== MASTER_SS_ID) {
    const cleanSs = String(ssId).trim();
    const isGoogleSheetId = /^[a-zA-Z0-9-_]{25,}$/.test(cleanSs) && !cleanSs.includes('@');
    if (!isGoogleSheetId) {
      console.log("[API] ssId is a contact handle (" + cleanSs + "), resolving...");
      const user = await findUserByContactInMaster(sheets, cleanSs);
      if (user && user.ssId) {
        ssId = user.ssId;
        if (query.ssId) query.ssId = ssId;
        if (parsedBody.ssId) parsedBody.ssId = ssId;
        console.log("[API] Resolved contact " + cleanSs + " to sheet ID: " + ssId);
      } else {
        console.log("[API] Could not resolve contact " + cleanSs + " in Master Sheet");
      }
    }
  }
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      try {
        ...
        // Return 200 with empty lists so the frontend can trigger onboarding!
        if ((status === 400 || status === 404) && e.message && (e.message.includes('Unable to parse range') || e.message.includes('not found') || e.message.includes('Requested entity was not found'))) {
          console.log(`[API] Configs range/tab is missing for ${targetSsId} (accessible sheet). Returning empty data for onboarding.`);
    ...
    if (method === 'POST') {
      ...
      if (payload.action === 'initTable') {
        const isGoogleSheetId = /^[a-zA-Z0-9-_]{25,}$/.test(targetSsId) && !targetSsId.includes('@');
        if (!isGoogleSheetId) {
          return res.status(400).json({ status: "error", message: "Некорректный Spreadsheet ID" });
        }
        let initOk = false;
        let initError = "";
        try {
