...
export const IncomeModal: React.FC<Props> = ({ isOpen, income, onClose, onSave, onDelete }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-6 animate-in fade-in duration-500">
            <div className="glass-panel w-full max-w-sm p-8 flex flex-col gap-6 shadow-2xl animate-in zoom-in-95 duration-500 text-white text-left">
                ...
                <div className="flex flex-col gap-4 text-white">
                    ...
                    <div className="flex flex-col gap-2 text-left text-white">
                        ...
                        <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-1 text-white text-left">
                            {INCOME_ICONS.map(i => (
                                <button key={i} onClick={() => setIcon(i)} className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border-2 transition-all duration-300 ${icon === i ? "border-[#10b981] bg-[#10b981]/10" : "border-white/5"}`}>
                    ...
                    <div className="flex flex-col gap-2 text-left text-white">
                        ...
                        <div className="flex justify-between text-white">
                            {COLORS.map(c => (
                                <button key={c} onClick={() => setColor(c)} className={`w-8 h-8 rounded-full border-2 transition-all duration-300 ${color === c ? "border-white scale-110" : "border-transparent opacity-50"}`} style={{ backgroundColor: c }} />
                            ))}
                        </div>
                    </div>
                </div>
