        ...
        {isSettingsMenuOpen && (
          <>
            ...
            <div className="absolute bottom-[72px] right-0 w-48 bg-[var(--bg-color)] border border-[var(--glass-border)] rounded-[24px] shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex flex-col z-[145] p-2.5 animate-in fade-in slide-in-from-bottom-5 zoom-in-95 origin-bottom-right backdrop-blur-xl">
              ...
              <button onClick={() => { setCalendarAnalyticsModal({ isOpen: true }); requestAnimationFrame(() => setIsSettingsMenuOpen(false)); }} className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-[var(--glass-item-bg)] transition-colors text-left"><Calendar size={15} className="text-emerald-500" /><span className="text-xs font-black text-[var(--text-main)] uppercase tracking-wider">Календарь</span></button>
              <button onClick={() => { setAnalyticsModal({ isOpen: true, type: "expense" }); requestAnimationFrame(() => setIsSettingsMenuOpen(false)); }} className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-[var(--glass-item-bg)] transition-colors text-left"><PieChart size={15} className="text-amber-500" /><span className="text-xs font-black text-[var(--text-main)] uppercase tracking-wider">Аналитика</span></button>
              {activeTableId && (
