...
export const DraggableIncomeItem: React.FC<Props> = ({
  ...
  return (
    <div
      ...
      className={`flex flex-col items-center justify-start transition-opacity w-[84px] shrink-0 cursor-pointer ${isDragging ? "opacity-30" : "opacity-100"} ${(isSortingMode && (isDragging || isPressing)) ? 'animate-wiggle' : ''}`}
    >
      <div
        {...listeners}
