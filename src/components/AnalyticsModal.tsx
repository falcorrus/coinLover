...
export const AnalyticsModal: React.FC<AnalyticsModalProps> = ({ 
    isOpen, onClose, categories, incomes, globalTransactions, initialType = "expense", onItemClick 
}) => {
    ...
    // Use sourceAmountUSD for expenses, and TargetUSD (or equivalent) for incomes
    ...
    if (tab === "categories" || analysisType === "income") {
        ...
        filteredTx.forEach(t => {
            // Expenses group by Category (targetId), Incomes group by IncomeSource (targetId)
            const id = t.targetId;
                    ...
                    {listItems.length === 0 && !isLoading ? (
                        ...
                        <div className="flex flex-col gap-4">
                            {listItems.map(item => {
                                ...
                                if (isExpanded) {
                                    const itemTx = filteredTx.filter(t => t.targetId === item.id);
                                    const tagMap = new Map<string, number>();
                                    itemTx.forEach(t => {
                                        const tagName = t.tag && t.tag.trim() ? t.tag.trim() : "Без тега";
                                        tagMap.set(tagName, (tagMap.get(tagName) || 0) + (t.sourceAmountUSD || 0));
                                    ...
                                    <div key={item.id} className="flex flex-col">
                                        ...
                                        {isExpanded && itemDetails.length > 0 && (
                                            <div className="mt-2 ml-11 flex flex-col gap-3 border-l-2 border-[var(--glass-border)] pl-4 animate-in slide-in-from-top-2 duration-300">
                                                {itemDetails.map(tag => (
                                                    <div key={tag.name} className="flex justify-between items-center cursor-pointer group" onClick={(e) => { e.stopPropagation(); if (onItemClick) onItemClick({ ...item, name: tag.name, id: tag.name }, "tag", filteredTx.filter(t => t.targetId === item.id && (t.tag?.trim() || "Без тега") === tag.name)); }}>
                                                        <div className="flex items-center gap-2"><Tag size={10} className="text-[var(--text-muted)]" /><span className="text-xs font-medium text-[var(--text-muted)] group-hover:text-[var(--text-main)] transition-colors">{tag.name}</span></div>
                                                        <div className="flex items-center gap-2"><span className={`text-xs font-bold ${analysisType === 'income' ? 'text-[var(--success-color)]' : 'text-[var(--text-main)]'}`}>${tag.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span><span className="text-[9px] font-bold text-[var(--text-muted)] w-8 text-right">{tag.percent.toFixed(0)}%</span></div>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
