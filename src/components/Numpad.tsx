    ...
    <div className="fixed inset-0 z-[150] flex flex-col bg-[var(--bg-color)] animate-in slide-in-from-right duration-500 ease-in-out font-sans">
      <div className="flex justify-between items-center px-4 py-4 bg-[var(--glass-bg)] border-b border-[var(--glass-border)] text-[var(--text-main)]">
        <div className="flex items-center gap-1">
          ...
          {isEditing && onRemove && (<button onClick={onRemove} className="p-2 text-[var(--danger-color)] hover:opacity-80 transition-colors"><Trash2 size={20} /></button>)}
        </div>
            ...
            <div className="flex items-center gap-1.5 absolute bottom-5 right-6">
              {data.type === 'income' ? (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border transition-all duration-300 active:scale-95 ${data.activeField === "source" ? 'bg-[var(--success-color)] text-white border-[var(--success-color)] shadow-lg' : 'bg-[var(--glass-item-bg)] border-[var(--glass-border)] text-[var(--text-muted)]'} hover:scale-105`}
                >
            ...
            <div className="flex items-center gap-1.5 absolute bottom-5 right-6">
              {data.type === 'expense' ? (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border transition-all duration-300 active:scale-95 ${data.activeField === "destination" ? 'bg-[#D4AF37] text-white border-[#D4AF37] shadow-lg' : 'bg-[var(--glass-item-bg)] border-[var(--glass-border)] text-[var(--text-muted)]'} hover:scale-105`}
                >
      ...
      <div className="flex items-center px-4 py-3 gap-3 bg-[var(--glass-bg)] shrink-0 border-t border-[var(--glass-border)] overflow-hidden">
        <div className="flex-1 flex items-center gap-2 overflow-x-auto hide-scrollbar">
          {data.type === 'expense' && data.destination && (data.destination as Category).tags && (data.destination as Category).tags.map(t => (
            <button key={t} onClick={() => handleTagClick(t)} className={`px-4 py-1.5 rounded-full uppercase text-[10px] font-black whitespace-nowrap transition-all duration-200 ${data.tag === t ? "bg-[var(--success-color)] text-white scale-105 shadow-md" : "bg-[var(--glass-item-bg)] text-[var(--text-muted)] border border-[var(--glass-border)] hover:bg-[var(--glass-item-active)]"}`}>{t}</button>
          ))}
        </div>
        ...
        <div className="mt-6 flex justify-center">
          {isEditing ? (<button onClick={() => onSubmit()} className="px-8 py-3 rounded-full text-sm font-black uppercase text-white bg-[var(--primary-color)] shadow-lg active:scale-95 tracking-wider">✓ Сохранить</button>) : (
            <div className="flex items-center bg-[var(--panel-bg)] rounded-full p-1 border border-[var(--glass-border)]">
              <button onClick={handleYesterday} className="px-6 py-2.5 text-xs font-bold tracking-wider text-[var(--text-muted)] rounded-full active:bg-[var(--glass-item-bg)]">Вчера</button>
              <button onClick={() => onSubmit()} className="px-6 py-2.5 bg-[var(--success-color)] text-white text-xs font-bold tracking-wider rounded-full shadow-sm active:scale-95">Сегодня</button>
