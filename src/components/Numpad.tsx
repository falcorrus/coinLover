    ...
    <div className="fixed inset-0 z-[150] flex flex-col bg-[var(--bg-color)] animate-in slide-in-from-right duration-500 ease-in-out">
      <div className="flex justify-between items-center px-4 py-4 bg-[var(--glass-bg)] border-b border-[var(--glass-border)] text-[var(--text-main)]">
        <div className="flex items-center gap-1">
          ...
          {isEditing && onRemove && (
            <button onClick={onRemove} className="p-2 text-[var(--danger-color)] hover:opacity-80 transition-colors"><Trash2 size={20} /></button>
          )}
        </div>
      ...
      <div className="flex items-center px-4 py-3 gap-3 bg-[var(--glass-bg)] shrink-0 border-t border-[var(--glass-border)] overflow-hidden">
        <div className="flex-1 flex items-center gap-2 overflow-x-auto hide-scrollbar">
          {data.type === 'expense' && data.destination && (data.destination as Category).tags && (data.destination as Category).tags.map(t => (
            <button key={t} onClick={() => handleTagClick(t)} className={`px-4 py-1.5 rounded-full uppercase text-[10px] font-black whitespace-nowrap transition-all duration-200 ${data.tag === t ? "bg-[var(--success-color)] text-white scale-105 shadow-md shadow-[var(--success-color)]/20" : "bg-[var(--glass-item-bg)] text-[var(--text-muted)] border border-[var(--glass-border)] hover:bg-[var(--glass-item-active)]"}`}>{t}</button>
          ))}
        </div>
        <button onClick={() => setIsCommentOpen(true)} className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 ${hasComment ? "bg-[var(--primary-color)]/20 text-[var(--primary-color)]" : "bg-[var(--glass-item-bg)] text-[var(--text-muted)] hover:text-[var(--text-main)] border border-[var(--glass-border)] shadow-sm"}`}><MessageSquare size={18} /></button>
      </div>

      <div className="bg-[var(--numpad-bg)] p-4 pb-8 border-t border-[var(--glass-border)]">
        <div className="flex gap-3">
          <div className="grid grid-cols-3 gap-2 flex-[2]">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (<button key={num} onClick={() => onPress(num.toString())} className="h-14 flex items-center justify-center text-3xl font-light text-[var(--text-main)] hover:bg-[var(--glass-item-bg)] rounded-xl transition-all active:scale-95">{num}</button>))}
            ...
            <button onClick={onDelete} className="h-14 flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--glass-item-bg)] rounded-xl transition-all active:scale-95"><Delete size={24} /></button>
          </div>
          <div className="flex-[1] bg-[var(--panel-bg)] rounded-2xl flex flex-col p-2 gap-2 shadow-inner">
            ...
            <button onClick={() => onPress("=")} className="h-12 w-full bg-[var(--btn-equals)] rounded-xl flex items-center justify-center text-2xl font-light text-[var(--text-main)] hover:brightness-95 transition-all active:scale-[0.98]">=</button>
          </div>
        </div>
        <div className="mt-6 flex justify-center">
          {isEditing ? (<button onClick={() => onSubmit()} className="px-8 py-3 rounded-full text-sm font-black uppercase text-white bg-[var(--primary-color)] shadow-lg transition-all active:scale-95 tracking-wider">✓ Сохранить изменения</button>) : (
      ...
      {isCommentOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[300] flex items-end justify-center animate-in fade-in duration-300">
          <div className="w-full max-w-md bg-[var(--bg-color)] border border-[var(--glass-border)] rounded-t-3xl p-6 flex flex-col gap-4 animate-in slide-in-from-bottom duration-300 shadow-2xl shadow-[var(--shadow-color)]">
            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><MessageSquare size={16} className="text-[var(--primary-color)]" /><h3 className="text-sm font-bold uppercase text-[var(--text-main)]">Комментарий</h3></div><button onClick={() => setIsCommentOpen(false)} className="p-1 text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors"><X size={20} /></button></div>
            <textarea autoFocus value={commentDraft} onChange={e => setCommentDraft(e.target.value)} placeholder="Заметка к транзакции..." rows={3} className="bg-[var(--glass-item-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-3 outline-none text-[var(--text-main)] resize-none text-sm focus:border-[var(--primary-color)]/50 transition-all" />
            <div className="flex gap-3"><button onClick={() => { setCommentDraft(""); onCommentChange(""); setIsCommentOpen(false); }} className="flex-1 h-12 rounded-xl bg-[var(--glass-item-bg)] border border-[var(--glass-border)] text-[var(--text-muted)] font-bold text-sm hover:text-[var(--danger-color)] transition-colors">ОЧИСТИТЬ</button><button onClick={handleCommentSave} className="flex-1 h-12 rounded-xl bg-[var(--primary-color)] text-white font-bold shadow-lg text-sm active:scale-95 transition-all">СОХРАНИТЬ</button></div>
          </div>
        </div>
      )}
    </div>
